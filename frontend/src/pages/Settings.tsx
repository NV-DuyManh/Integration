...
export default function Settings() {
  ...
  const [toggles, setToggles] = useState({ darkMode: true, notifications: false, autoSync: true });
    ...
    <div className="card glass fade-in" style={{ borderRadius: 'var(--radius-lg)' }}>
      ...
      <div className="card-body" style={{ padding: 32 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, marginBottom: 32 }}>
          {/* Account Info */}
          <div style={{ border: '1px solid rgba(0,242,254,0.08)', borderRadius: 'var(--radius-md)', padding: 24, background: 'rgba(15,23,42,0.4)' }}>
          ...
          <div style={{ border: '1px solid rgba(0,242,254,0.08)', borderRadius: 'var(--radius-md)', padding: 24, background: 'rgba(15,23,42,0.4)' }}>
            ...
            <Toggle label="Dark Mode Theme" checked={toggles.darkMode} onChange={() => setToggles({...toggles, darkMode: !toggles.darkMode})} />
            <Toggle label="Email Notifications" checked={toggles.notifications} onChange={() => setToggles({...toggles, notifications: !toggles.notifications})} />
            <Toggle label="Auto-Sync HR Data" checked={toggles.autoSync} onChange={() => setToggles({...toggles, autoSync: !toggles.autoSync})} />
          </div>
        </div>
